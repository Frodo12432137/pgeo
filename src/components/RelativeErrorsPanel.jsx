...
const RelativeErrorsPanel = ({ data }) => {
    const chartData = useMemo(() => {
        ...
        const locations = {};
        data.forEach(row => {
            // MAPE is already calculated per day in mock data, let's just average it here for the selected period
            if (!locations[row.lokalizacja]) {
                locations[row.lokalizacja] = { name: row.lokalizacja, sumKorekta: 0, sumHres: 0, count: 0 };
            }
            locations[row.lokalizacja].sumKorekta += row.MAPE_Korekta_Doba;
            locations[row.lokalizacja].sumHres += row.MAPE_HRES_Doba;
            locations[row.lokalizacja].count += 1;
        });

        return Object.values(locations).map(loc => ({
            name: loc.name,
            Korekta: Number(((loc.sumKorekta / loc.count) * 100).toFixed(2)),
            HRES: Number(((loc.sumHres / loc.count) * 100).toFixed(2))
        }));
    }, [data]);

    return (
        <div className="glass-panel col-span-12 lg:col-span-6 flex flex-col">
            <h3 className="text-lg font-semibold mb-4 text-primary">Błąd Względny (MAPE)</h3>
...
export default RelativeErrorsPanel;
